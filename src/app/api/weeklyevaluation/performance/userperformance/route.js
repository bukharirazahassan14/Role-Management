...
export async function GET(req) {
  try {
    ...
    // Build a filter for User collection
    const userMatch = {};
    if (userIdParam) {
      userMatch._id = new mongoose.Types.ObjectId(userIdParam); // ✅ filter by userId
    }

    const results = await User.aggregate([
      // ✅ Optional match stage for specific user
      { $match: userMatch },
...

      {
        $project: {
          ...
          avgWeightedRating: {
            $ifNull: ["$evaluations.avgWeightedRating", 0],
          },
          performance: {
            $cond: {
              if: {
