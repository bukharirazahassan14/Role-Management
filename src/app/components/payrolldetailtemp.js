...
const initialCompensationData = [
  // Base Salary (Initial Value set high for visibility)
  { name: "Base Salary", value: 500000, type: "EARNING" },
  // Map templates to initial items
  ...ALLOWANCE_TEMPLATES.map((name) => ({
    name,
    value: name.includes("Rent") || name.includes("Transport") ? 15000 : 0, // Initial values for some common ones
    enabled: name.includes("Rent") || name.includes("Transport"),
    type: "ALLOWANCE",
  })),
  ...DEDUCTION_TEMPLATES.map((name) => ({
    name,
    value: name.includes("Tax") || name.includes("Fund") ? 20000 : 0, // Initial values for some common ones
    enabled: name.includes("Tax") || name.includes("Fund"),
    type: "DEDUCTION",
  })),
];
...
const NewItemModal = ({ isOpen, type, onClose, onSave, existingNames }) => {
  ...
  const handleSave = () => {
    ...
    if (existingNames.includes(trimmedName)) {
      setError(`Item named "${trimmedName}" already exists.`);
      return;
    }
    ...
    // Pass the new item object to the main handler
    onSave({
      name: trimmedName,
      value: numericValue,
      enabled: true, // New items are enabled by default
      type: type,
    });
    onClose();
  };
...
function PayrollConfig({
  ...
  onAddAllowanceClick,
  onAddDeductionClick,
  onDeleteAllowance, // New prop
  onDeleteDeduction, // New prop
}) {
  const data = compensationData;

  // States for dropdown selections (Unchanged)
    ...
    <div className="p-8 bg-white rounded-3xl shadow-[0_20px_50px_rgba(109,40,217,0.1)] border border-gray-100 font-sans mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        ...
        <div className="md:col-span-2 space-y-6">
          {/* Configuration Card: Employment Type */}
          <ConfigCard
          ...
          {/* Configuration Card: Payroll Frequency */}
          <ConfigCard
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8 md:p-12 font-inter">
      ...
      <div className="max-w-8xl mx-auto">
        ...
        <PayrollConfig
          ...
          onDeleteAllowance={handleDeleteItem} // Pass delete handler
          onDeleteDeduction={handleDeleteItem} // Pass delete handler
        />
